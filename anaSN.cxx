...
//    gStyle->SetOptStat(0);
    ...
    for(int j = 0; j < 18; j++){
        ...
        sprintf(noise_name, "%d",  j+100);
        hist[j] = new TH1F(hist_name, hist_name, 300, 0, 3000);
        noise[j] = new TH1F(noise_name, noise_name, 200, -10, 10);
    }
        ...
        for(int ch = 0; ch < CHNUM; ch++){
            ...
            if(readnum != (unsigned int)CH_FOOTERSIZE){
				...
				for(int ch = 0; ch < CHNUM; ch++){
					//for Ext. trig
					float wf_ptr[100];
					for(int gate=50;gate<800;gate++){
						b+=get_data_final[ch][gate];
						noise[ch]->Fill(get_data_final[ch][gate]);
					}
					b/=750;
					bRMS=(float)noise[ch]->GetRMS();
					noise[ch]->Reset("ICESM");
					for(int gate=800;gate<900;gate++){
						wf_ptr[gate-800]=get_data_final[ch][gate]-bRMS;
					}
					max=TMath::MaxElement(100,wf_ptr);
					hist[ch]->Fill(max/bRMS);
					b=0;
					/*-----------------------------*/
					//for base mode
...
//					hist[ch]->Fill(max-min);
    ...
    for(int ch = 0; ch < 1; ch++){
        ...
        //float mean = (float)hist[ch]->FindFirstBinAbove(120);
		...
		Canvas->SaveAs("pulse_height.png");
//    Canvas->Update();
