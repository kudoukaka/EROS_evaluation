...
int rbcp_com(const char* ipAddr, unsigned int port, struct rbcp_header* sendHeader, const char* sendData, char* recvData, char dispMode, unsigned char* parameter){
  ...
  while(numReTrans<100){
    ...
    int select_no = select(sock+1, &setSelect, NULL, NULL,&timeout);
    if(select_no == -1){
        if(errno == EINTR){
        printf("EINTR \n");
        ;   //retry
	...
	if(dispMode==RBCP_DISP_MODE_DEBUG){
	  ...
	  if(sendHeader->command==RBCP_CMD_RD){
        ...
        */
            *(parameter + j) = (unsigned char)rcvdBuf[i];
	        j++;
	    }
	    //if(j!=15) puts(" ");
	  }else{
